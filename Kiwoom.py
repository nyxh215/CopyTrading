...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self.code_info = pd.DataFrame(columns=("code_id","code_nm","market_cap"))
        self.main_info = pd.DataFrame(columns=("Open","High","Low","Close","Volume","code_id"))
        self.oneday = (date.today() - timedelta(2))
        self.maxday = ''
...
# COM을 사용하기 위한 메서드
    ...
    def _comm_get_data(self, code, real_type, field_name, index, item_name):
        ...
        #ret = self.dynamicCall("GetCommData(QString, QString, QString, int, QString)", code, real_type, field_name,index, item_name)
    ...
    def _receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3, unused4):
        ...
        if rqname == "opt10081_req":
            ...
            self._opt10001_20006(rqname, trcode)
        try:
    ...
    # 종목의 일자, 시가, 종가 데이터를 요청
    def req_stock_daily_main(self, code, day):
        self.main_info = pd.DataFrame(columns=("Open", "High", "Low", "Close","Volume","code_id"))
        self.maxday = day
        ...
        if day is not None:
            day = day.strftime('%Y%m%d')
            temp_data = self.main_info[self.main_info.index.values <= day]
            if temp_data.empty is False:
                self.main_info = self.main_info[self.main_info.index.values>day]
                self.remained_data = False

        if self.oneday == self.maxday:
            self.main_info = self.main_info[self.main_info.index.values > day]
            self.remained_data = False
        ...
        self.main_info.index.name = "Date"
    ...
    def _opt10081_20006(self,rqname,trcode):
        ...
        #print(type(self.oneday))
        #print(type(self.maxday))
        ...
        if data_cnt == 0:
            ...
            self.main_info = self.main_info.append(pd.DataFrame({'Open':open, 'Low':low, 'High':High,'Close':close, 'Volume':volume,'code_id':code}, index=[date]))
            self.stock_info = self.stock_info.append(pd.DataFrame({'Open': open, 'Low': low, 'High': High, 'Close': close, 'Volume': volume},index=[date]))

            #print("Download date :", date, "Download Code :", code)

        elif self.oneday == self.maxday:
#            pass
            date = self._comm_get_data(trcode, "", rqname, 0, "일자")
            open = self._comm_get_data(trcode, "", rqname, 0, "시가")
            low = self._comm_get_data(trcode, "", rqname, 0, "저가")
    ...
    # 종목 기본 데이터를 서버에서 가져오고 code_info 에 저장
    def _opt10001_20006(self,rqname,trcode):
        data_cnt = self._get_repeat_cnt(trcode, rqname)
        if data_cnt == 0:
            code = self._comm_get_data(trcode, "", rqname, 0, "종목코드")
            name = self._comm_get_data(trcode, "", rqname, 0, "종목명")
            marketCap = self._comm_get_data(trcode, "", rqname, 0, "시가총액")
            self.code_info = self.code_info.append(pd.DataFrame({'code_nm':name,'market_cap':marketCap}, index=[code]))
        else:
            for i in range(data_cnt):
                code = self._comm_get_data(trcode, "", rqname, i, "종목코드")
                name = self._comm_get_data(trcode, "", rqname, i, "종목명")
                marketCap = self._comm_get_data(trcode, "", rqname, i, "시가총액")
                self.code_info = self.code_info.append(pd.DataFrame({'code_nm':name,'market_cap':marketCap}, index=[code]))
    ...
    # 종목 기본 데이터를 요청
    def req_stock_base_value(self, code):
        self.code_info = pd.DataFrame(columns=("code_nm","market_cap"))

        print("종목 기본 데이터 요청")
        self.set_input_value("종목코드", code)
        self.comm_rq_data("opt10001_req", "opt10001", 0, "2001")

        while self.remained_data:
            print("종목 기본 데이터 추가 요청")
            #time.sleep(0.2)
            self.set_input_value("종목코드", code)
            self.comm_rq_data("opt10001_req", "opt10001", 2, "2001")

        self.code_info.index.name = "code_id"
        return self.code_info

    # 종목 기본 데이터를 요청
    def req_stock_allCode_value(self):
        self.code_info = pd.DataFrame(columns=("code_nm","market_cap"))
        ...
        for x in kospi_code_list:
            print("종목코드" + x + "에 대한 데이터 요청")
            self.set_input_value("종목코드", x)
            #self.comm_rq_data("opt10001_req", "opt10001", 0, "2001")
            code = x
            name = self.dynamicCall("GetMasterCodeName(QString)", [x])
            stock_cnt = self.dynamicCall("GetMasterListedStockCnt(QString)",[x])
            stock_price = self.dynamicCall("GetMasterLastPrice(QString)", [x])

            market_cap = round(float(stock_cnt) * float(stock_price) / 100000000)
            #print(stock_cnt)
            #print(stock_price)
            #print(market_cap)
            self.code_info = self.code_info.append(pd.DataFrame({'code_nm': name, 'market_cap': market_cap}, index=[code]))
            if x == kospi_code_list[-2]:
                break
            #else:
            #    time.sleep(3.7)
